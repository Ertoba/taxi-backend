...
exit; */
    ...
    if(! $result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        foreach($_POST['car_type'] as $cartype_id){
            ...
            $hourly_rate = isset($_POST['hourlyrate-' . $cartype_id]) ? 1 : 0;
            ...
            $wt = (int) $_POST['wt-' . $cartype_id];
            $wcpmr = (float) $_POST['wcpmr-' . $cartype_id];
            $hr_cph = isset($_POST['cph-' . $cartype_id]) ? (float) $_POST['cph-' . $cartype_id] : 0;
            $hr_dist = isset($_POST['cphdist-' . $cartype_id]) ? (float) $_POST['cphdist-' . $cartype_id] : 1;
            ...
            $nwt = (int) $_POST['nwt-' . $cartype_id];
            $nwcpmr = (float) $_POST['nwcpmr-' . $cartype_id];
            $nhr_cph = isset($_POST['ncph-' . $cartype_id]) ? (float) $_POST['ncph-' . $cartype_id] : 0;
            $nhr_dist = isset($_POST['ncphdist-' . $cartype_id]) ? (float) $_POST['ncphdist-' . $cartype_id] : 1;
            ...
            $DB_values_string .= "({$hourly_rate},{$hr_cph},{$hr_dist},{$nhr_cph},{$nhr_dist},{$wt},{$wcpmr},{$nwt},{$nwcpmr},{$cartype_id},%2\$d,{$cc},{$cpkr},{$cpmr},{$pcr},{$dcr},{$ncc},{$ncpkr},{$ncpmr},{$npcr},{$ndcr},{$pp_enabled},{$pp_start_time},{$pp_end_time},{$pp_active_days},{$pp_charge_type},{$pp_charge},{$fare_compute},{$ride_share},{$ind},{$nind},\"{$alt_cars}\"),";
        ...
        $query = sprintf('INSERT INTO %1$stbl_rides_tariffs (hr_enabled,hr_cph,hr_dist,nhr_cph,nhr_dist,wait_time,wait_cost_per_minute,nwait_time,nwait_cost_per_minute,ride_id,routes_id,cancel_cost,cost_per_km,cost_per_minute,pickup_cost,drop_off_cost,ncancel_cost,ncost_per_km,ncost_per_minute,npickup_cost,ndrop_off_cost,pp_enabled,pp_start,pp_end,pp_active_days,pp_charge_type,pp_charge_value,cfare_enabled,rshare_enabled,init_distance,init_distance_n,alt_cars) VALUES '.
        $DB_values_string, 
        DB_TBL_PREFIX, 
        $id   
        );
