...
if(!empty($_GET['filter-records']) && isset($_GET['document-status']) && $_GET['document-status'] != "---"){
    ...
    INNER JOIN %1$stbl_users_documents ON %1$stbl_users_documents.u_id = %1$stbl_drivers.driver_id AND %1$stbl_users_documents.u_type = 1
    WHERE %1$stbl_drivers.driver_id %2$s GROUP BY %1$stbl_users_documents.u_doc_status', DB_TBL_PREFIX,$query_modifier, LOCATION_INFO_VALID_AGE);  //Get and count all data
...
if(!empty($_GET['filter-records']) && isset($_GET['document-status']) && $_GET['document-status'] != "---"){
    ...
    $query = sprintf('SELECT COUNT(DISTINCT %1$stbl_drivers.driver_id) AS `COUNT(*)` FROM %1$stbl_drivers
    LEFT JOIN %1$stbl_driver_location ON %1$stbl_driver_location.driver_id = %1$stbl_drivers.driver_id
    INNER JOIN %1$stbl_users_documents ON %1$stbl_users_documents.u_id = %1$stbl_drivers.driver_id AND %1$stbl_users_documents.u_type = 1
    WHERE %1$stbl_drivers.driver_id %2$s', DB_TBL_PREFIX,$query_modifier, LOCATION_INFO_VALID_AGE);  //Get and count all data
...
if(!empty($_GET['filter-records']) && isset($_GET['document-status']) && $_GET['document-status'] != "---"){
    ...
    INNER JOIN %1$stbl_users_documents ON %1$stbl_users_documents.u_id = %1$stbl_drivers.driver_id AND %1$stbl_users_documents.u_type = 1
    WHERE %1$stbl_drivers.driver_id %4$s GROUP BY %1$stbl_drivers.driver_id ORDER BY %5$s LIMIT %2$d,%3$d', DB_TBL_PREFIX, $offset, ITEMS_PER_PAGE, $query_modifier, $sort_by);
