...
if(isset($_POST['savesettings'])){
    ...
    if(isset($_POST['update-all-drivers'])){

        $driver_commission = floatval($_POST['driver-default-commission']);
        $city_scope = (int) $_POST['update-drivers-commission-scope'];

        if($city_scope == 0){
            $query = sprintf('UPDATE %stbl_drivers SET driver_commision = %f', DB_TBL_PREFIX, $driver_commission);
        }else{
            $query = sprintf('UPDATE %stbl_drivers SET driver_commision = %f WHERE route_id = %d', DB_TBL_PREFIX, $driver_commission, $city_scope);
        } 
...
if(isset($_POST['savesettings2'])){
    ...
    if(isset($_POST['paystack-pub-key'])){
        $payment_gateway_data['paystack'] = [
                                                'paystack-pub-key' => $_POST['paystack-pub-key'],
                                                'paystack-secret-key' => $_POST['paystack-secret-key']
                                            ];
    }


    if(isset($_POST['paypal-client-id'])){
        $payment_gateway_data['paypal'] = [
                                                'paypal-client-id' => $_POST['paypal-client-id'],
                                                'paypal-secret-key' => $_POST['paypal-secret-key']
                                            ];
    }


    if(isset($_POST['pesapal-consumer-key'])){
        $payment_gateway_data['pesapal'] = [
                                                'pesapal-consumer-key' => $_POST['pesapal-consumer-key'],
                                                'pesapal-consumer-secret' => $_POST['pesapal-consumer-secret']
                                            ];
    }
    


    if(isset($_POST['stripe-pub-key'])){
        $payment_gateway_data['stripe'] = [
                                                'stripe-pub-key' => $_POST['stripe-pub-key'],
                                                'stripe-secret-key' => $_POST['stripe-secret-key']
                                            ];
    }


    if(isset($_POST['paytr-merchant-id'])){
        $payment_gateway_data['paytr'] = [
                                                'paytr-merchant-id' => $_POST['paytr-merchant-id'],
                                                'paytr-merchant-key' => $_POST['paytr-merchant-key'],
                                                'paytr-merchant-salt' => $_POST['paytr-merchant-salt']
                                            ];
    }


    if(isset($_POST['paytm-merchant-id'])){
        $payment_gateway_data['paytm'] = [
                                                'paytm-merchant-id' => $_POST['paytm-merchant-id'],
                                                'paytm-merchant-key' => $_POST['paytm-merchant-key']
                                            ];
    }


    if(isset($_POST['phonepe-merchant-id'])){
        $payment_gateway_data['phonepe'] = [
                                                'phonepe-merchant-id' => $_POST['phonepe-merchant-id'],
                                                'phonepe-merchant-salt' => $_POST['phonepe-merchant-salt']
                                            ];
    }


    if(isset($_POST['flutterwave-pub-key'])){
        $payment_gateway_data['flutterwave'] = [
                                                    'flutterwave-pub-key' => $_POST['flutterwave-pub-key'],
                                                    'flutterwave-secret-key' => $_POST['flutterwave-secret-key']
                                                ];
    }


    if(isset($_POST['payku-public-token'])){
        $payment_gateway_data['payku'] = [
                                                'payku-public-token' => $_POST['payku-public-token'],
                                                'payku-private-token' => $_POST['payku-private-token']
                                            ];
    }


    if(isset($_POST['paymob-iframe-id'])){
        $payment_gateway_data['paymob'] = [
                                                'paymob-iframe-id' => $_POST['paymob-iframe-id'],
                                                'paymob-api-key' => $_POST['paymob-api-key'],
                                                'paymob-int-id' => $_POST['paymob-int-id']
                                            ];
    }


    if(isset($_POST['midtrans-merchant-id'])){
        $payment_gateway_data['midtrans'] = [
                                                'midtrans-merchant-id' => $_POST['midtrans-merchant-id'],
                                                'midtrans-client-key' => $_POST['midtrans-client-key'],
                                                'midtrans-server-key' => $_POST['midtrans-server-key']
                                            ];
    }


    $_POST['default-payment-gateway'] = $payment_gateway_data;
