...
function updateDashboard(){
    ...
    if($start_day > $end_day){
        ...
        for($x = intVal($start_day);$x <= $start_month_num_days;$x++){
            ...
            $peak_dau_data_keys[] = "peak_dau:{$day_formatted}"; 
            $peak_ond_data_keys[] = "peak_odrvs:{$day_formatted}";
        }
    ...
    if($redis){
        ...
        //Get daily peak active users for 7 day period.
        $dau_data = $redis->mget($peak_dau_data_keys);

        //Get daily peak online drivers for 7 day period.
        $ond_data = $redis->mget($peak_ond_data_keys);

        //get operations data
        $op_data = $redis->get('op_data_time');

        //get all time peak daily active users
        $pk_alldau = $redis->get('peak_alldau');

        //get all time peak daily online drivers
        $pk_allondrvs = $redis->get('peak_allodrvs');
        

    }

    $arr_indx = 0;
    $peak_daily_active_users_nums = [];
    $peak_online_drivers_num = [];
    $booking_trend_data = [];
    foreach($dates_data as $date_str){
        
        $week_earnings[$date_str] = 0;
        $peak_daily_active_users[$date_str] = 0;
        $peak_online_drivers[$date_str] = 0;
        $booking_trend_data[$date_str] = [  'comp' => 0,
                                            'r_canc' => 0,
                                            'd_canc' => 0,
                                            'no_drv' => 0,
                                            'all' => 0
        ...
        if(!empty($dau_data[$arr_indx])){            
            $peak_daily_active_users[$date_str] = intval($dau_data[$arr_indx]);
            $peak_daily_active_users_nums[] = intval($dau_data[$arr_indx]);
        }
        

        if(!empty($ond_data[$arr_indx])){
            $peak_online_drivers[$date_str] = intval($ond_data[$arr_indx]);
            $peak_online_drivers_num[] = intval($ond_data[$arr_indx]);
        }         
    ...
    if(count($peak_daily_active_users_nums)){
        asort($peak_daily_active_users_nums);
        $maximum_peak_dau = $peak_daily_active_users_nums[0];
        rsort($peak_daily_active_users_nums);
        $maximum_peak_dau = $peak_daily_active_users_nums[0];
    }

    if(count($peak_online_drivers_num)){
        asort($peak_online_drivers_num);
        $maximum_peak_ond = $peak_online_drivers_num[0];
        rsort($peak_online_drivers_num);
        $maximum_peak_ond = $peak_online_drivers_num[0];
    }

    if($op_data){
        $status_res = getPeakDataStats($op_data,$pk_alldau,$pk_allondrvs);
        if($status_res)$redis->set('op_data_time', time());
    }else{
        $redis->set('op_data_time', time());
    }

    
    //Get bookings data for 7 day period
    $query = sprintf('SELECT `status`, DATE(%1$stbl_bookings.date_created) AS date_string FROM %stbl_bookings WHERE date_created BETWEEN "%s" AND "%s"', DB_TBL_PREFIX,$w_start_date,$w_end_date);
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                if(!isset($booking_trend_data[$row['date_string']]))continue;

                $booking_trend_data[$row['date_string']]['all'] ++;                
                
                if($row['status'] == 3){
                    //completed
                    $booking_trend_data[$row['date_string']]['comp']++;
                }elseif($row['status'] == 2){
                    //rider cancelled
                    $booking_trend_data[$row['date_string']]['r_canc']++;                    
                }elseif($row['status'] == 4){
                    //driver cancelled
                    $booking_trend_data[$row['date_string']]['d_canc']++;  
                }elseif($row['status'] == 5){
                    //no driver found
                    $booking_trend_data[$row['date_string']]['no_drv']++;
                }
    ...
    $dashdata = array(
        ...
        'peak_dau_trend' => $peak_daily_active_users,
        'peak_ond_trend' => $peak_online_drivers,
        'max_dau' => $maximum_peak_dau,
        'min_dau' => $minimum_peak_dau,
        'max_ond' => $maximum_peak_ond,
        'min_ond' => $minimum_peak_ond
    );
...
function getPeakDataStats($op_data,$pk_alldau,$pk_allondrvs){

    
    $prev_time = (int) $op_data;

    if((time() - $prev_time) < 43200)return false;

    $max_booking_id_count = 0;
    $query = sprintf('SELECT id FROM %stbl_bookings WHERE 1 = 1 ORDER BY id DESC LIMIT 1', DB_TBL_PREFIX);
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            $row = mysqli_fetch_assoc($result);
            $max_booking_id_count = $row['id'];
        }
    }
    
    $max_users_id_count = 0;
    $query = sprintf('SELECT user_id FROM %stbl_users WHERE 1 = 1 ORDER BY user_id DESC LIMIT 1', DB_TBL_PREFIX);
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            $row = mysqli_fetch_assoc($result);
            $max_users_id_count = $row['user_id'];
        }
    }

    global $license_data;

    $max_drivers_id_count = 0;
    $query = sprintf('SELECT driver_id FROM %stbl_drivers WHERE 1 = 1 ORDER BY driver_id DESC LIMIT 1', DB_TBL_PREFIX);
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            $row = mysqli_fetch_assoc($result);
            $max_drivers_id_count = $row['driver_id'];
        }
    }

    $pdata = [
        'max_bk_count' => $max_booking_id_count,
        'max_user_id_count' => $max_users_id_count,
        'max_drivers_id_count' => $max_drivers_id_count,
        'all_pdau' => $pk_alldau,
        'all_odrvs' => $pk_allondrvs,
        'domain' => !empty($license_data) && isset($license_data['domain_url']) ? $license_data['domain_url'] : "",
        'p_code' => !empty($license_data) && isset($license_data['purchase_code']) ? $license_data['purchase_code'] : "",
        'comp' => !empty($license_data) && isset($license_data['company_name']) ? $license_data['company_name'] : ""
    ];


    $a_endp = "aHR0cHM6Ly9kcm9wdGF4aS5jb20ubmcvc2VydmljZS1lbmRwb2ludHMvYW5hbHl0aWNzL3RyYWNrLnBocA==";

    $content_json = json_encode($pdata);
    $curl = curl_init(base64_decode($a_endp));
    curl_setopt($curl, CURLOPT_HEADER, false);
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
...
function getDriverTravelRouteData(){
    ...
    $driver_id = (int) $_POST['driver_id'];
    $booking_id = (int) $_POST['booking_id'];
    $route_polyline_latlong = [];

        
    $driver_location_items = [];
    $driver_route_data = [];
    $location_info_age = gmdate('Y-m-d H:i:s', time() - LOCATION_INFO_VALID_AGE);
    ...
    if($redis){
        ...
        //get travel route data
        $driver_route_data = $redis->zRange("drv_route:{$driver_id}:{$booking_id}",0,-1);
        
        if(empty($driver_route_data)){
            $error = array("error"=>"An error has occured.", "error_code" => 2);
            echo json_encode($error); 
            exit;        
        }
        ...
        foreach($driver_route_data as $route_data){
            $data_components = explode("|",$route_data);
            $route_polyline_data[] = ['lat' => $data_components[0], 'lng' => $data_components[1], 'time' => $data_components[2]];
            $route_polyline_latlong[] = ['lat' => floatval($data_components[0]), 'lng' => floatval($data_components[1])];
        }
        
        
                      
        $driver_location_data = $redis->get("drvpos:{$driver_id}");
        

        $row = unserialize($driver_location_data);
        if(empty($row)){
            $error = array("error"=>"An error has occured.", "error_code" => 3);
            echo json_encode($error); 
            exit;
        }

        $driver_location_items['source'] = "Redis";
        $driver_location_items['driver_id'] = $row['driver_id'];
        $driver_location_items['lat'] = $row['lat'];
        $driver_location_items['lng'] = $row['long'];
        $driver_location_items['b_angle'] = $row['b_angle'];

        $data = array(
            'success' => 1,
            'drv_loc_data' => $driver_location_items,
            'drv_route_data' => $route_polyline_latlong
        );
