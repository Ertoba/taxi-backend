...
")->where('user_type', $userType)->first();
    ...
    {
        $verified = (bool) $request->boolean('document_verify');
        ...
        if (! $user->firestore_id) {
            ...
            if (! $firestoreDoc) {
                return response()->json(['message' => 'Unable to create the driver in Firestore.'], 502);
            }
            $firestoreDocId = basename($firestoreDoc);
            $user->update(['firestore_id' => $firestoreDocId]);
            $user['firestore_id'] = $firestoreDocId;
        }
        $firestoreUpdated = $this->updateDocument('drivers', $user->firestore_id, [
        ...
        if (! $firestoreUpdated) {
            return response()->json(['message' => 'Unable to update driver verification in Firestore.'], 502);
        }

        $user->update([
            'document_verify' => $verified ? 1 : 0,
            // host_status is ENUM('0', '1', '2'); bind the enum value as text.
            'host_status' => $verified ? '1' : '0',
        ]);
