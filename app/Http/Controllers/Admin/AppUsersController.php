    ...
    public function updateIdentify(Request $request, FirebaseAuthService $firebaseAuthService)
    {
        ...
        if (! $firestoreDocId || ! $firestoreDocExists) {
            ...
            if (! $firestoreDoc) {
                return response()->json([
                    'status' => 502,
                    'message' => 'Unable to create the driver in Firestore.',
                ], 502);
            }
        ...
        $firestoreUpdated = $this->updateDocument('drivers', $firestoreDocId, [
        ...
        if (! $firestoreUpdated) {
            return response()->json([
                'status' => 502,
                'message' => 'Unable to update driver verification in Firestore.',
            ], 502);
        }

        $user->update([
            'document_verify' => $verified ? 1 : 0,
            // host_status is ENUM('0', '1', '2'); bind string values so MySQL
            // does not interpret integers as enum indexes.
