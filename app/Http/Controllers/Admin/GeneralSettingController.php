...
class GeneralSettingController extends Controller
{
    ...
    public function ubillSetting()
    {
        ...
        return view('admin.generalSettings.smssettings.ubill', [
            'ubill_api_key' => $settings->get('ubill_api_key'),
            'ubill_brand_id' => $settings->get('ubill_brand_id'),
            'sms_provider_name' => $settings->get('sms_provider_name'),
            'auto_fill_otp' => $settings->get('auto_fill_otp'),
            'id' => 1,
        ]);
    }

    public function ubillUpdate(Request $request)
    {
        if (Gate::denies('general_setting_edit')) {
            return response()->json(['error' => 'Form submission is disabled in demo mode.'], 403);
        }
        ...
        $validated = $request->validate([
            'ubill_api_key' => ['nullable', 'string', 'max:255'],
            'ubill_brand_id' => ['nullable', 'string', 'max:100'],
        ]);

        foreach ($validated as $metaKey => $metaValue) {
            if (filled($metaValue)) {
                GeneralSetting::updateOrCreate(
                    ['meta_key' => $metaKey],
    ...
    public function pushNotificationUpdate(Request $request)
    {
        ...
        $request->validate([
            'onesignal_app_id' => ['required', 'uuid'],
            'onesignal_rest_api_key' => ['required', 'string', 'min:32', 'not_in:test'],
            'onesignal_app_id_driver' => ['required', 'uuid'],
            'onesignal_rest_api_key_driver' => ['required', 'string', 'min:32', 'not_in:test'],
        ]);
        GeneralSetting::updateOrCreate(
