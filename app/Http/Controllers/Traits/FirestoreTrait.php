...
trait FirestoreTrait
{
    ...
    public function generateDriverFirestoreData($customer)
    {
        ...
        $documentsApproved = (int) $customer->document_verify === 1;

        return array_merge([
            'completed_rides' => [],
            'docApprovedStatus' => $documentsApproved ? 'approved' : 'pending',
            'driverRating' => '',
            ...
            'isVerified' => $documentsApproved ? 'yes' : 'no',
            'rejected_rides' => [],
            'rideStatus' => 'available',
            // The driver app expects a Firestore map here, not an array.
            'ride_request' => new \stdClass,
            'timestamp' => now(),
        ], $this->generateDriverProfileFirestoreData($customer));
    }

    /**
     * Generate only profile and vehicle fields that are safe to refresh.
     * Runtime fields (online status, location and ride request) must be preserved.
     */
    public function generateDriverProfileFirestoreData($customer): array
    {
